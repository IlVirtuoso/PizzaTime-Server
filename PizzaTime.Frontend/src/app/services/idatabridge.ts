...
//Role doesn't exist
import { Ingredient, Order, Pizza, Pizzeria, Menu, User, OrderRow } from "@data";
...
export abstract class IDataBridge {
  ...
  public abstract addAdditionToMenu(
    additions: AddIngrRequest[]
  ): Promise<boolean>;
  public abstract getMenu(): Promise<Menu | null>;
  public abstract createMenu(): Promise<boolean>;
  public abstract getMenuRowsForOrder(order: {
    order: getMenuRowsForOrderRequest[];
  }): Observable<Menu[]>;
  ...
  public abstract createOrder(): Promise<Order>;
  public abstract submitOrder(order: Order): Promise<Order>;
  public abstract addOrderRow(
    order: Order,
    baseId: number,
    pizzaId: number,
    ingredients: number[],
